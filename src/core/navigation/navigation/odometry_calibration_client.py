...
class MoveToClient(Node):
    ...
    def send_request(self, path, enforce_orientation=True, allow_reverse=False, 
                   stop_at_goal=True, max_speed=0.3, max_turn_speed=2.0):
        """
    ...
    def create_path(self, waypoints, frame_id='map'):
        """
        ...
        for point in waypoints:
            ...
            pose.pose.orientation.x = 0.0
            pose.pose.orientation.y = 0.0
            pose.pose.orientation.z = np.sin(float(theta) / 2.0)
            pose.pose.orientation.w = np.cos(float(theta) / 2.0)
...
def main(args=None):
    ...
    path = move_to_client.create_path(waypoints)
    future = move_to_client.send_request(path, max_turn_speed=0.2, enforce_orientation=True)
