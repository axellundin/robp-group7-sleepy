...
class circle_creator:
    ...
    def circle_filler_angle_dependent(self, x,y,area,grid, radius,fill_in,edge_only, area_only, from_middle,v1,v2,with_obs):
        ...
        outside_area = False
        if area_only:
            if x >= area.x_start and y >= area.y_start and x <= area.x_end and y <= area.y_end:
        ...
        positive_angle_limit = v1
        negative_angle_limit = v2

        if positive_angle_limit and negative_angle_limit:

            if v2 < -np.pi:
                negative_angle_limit = v2 +2*np.pi
                positive_angle_limit = v1 +2*np.pi

            start_angles_obs1.append(start_v1)
            end_angles_obs1.append(positive_angle_limit)
            start_angles_obs1.append(negative_angle_limit)
            end_angles_obs1.append(0)

            start_angles_obs3.append(-np.pi)
            end_angles_obs3.append(negative_angle_limit)
            start_angles_obs3.append(positive_angle_limit)
            end_angles_obs3.append(np.pi)
            ...
            start_angles_obs2.append(-2*np.pi)
            end_angles_obs2.append(negative_angle_limit)
            start_angles_obs2.append(positive_angle_limit)
            end_angles_obs2.append(0)

            start_angles_obs4.append(np.pi)
            end_angles_obs4.append(positive_angle_limit)
            start_angles_obs4.append(negative_angle_limit)
            end_angles_obs4.append(-np.pi)
        ...
        if outside_area:
            ...
            point_on_circle[:] = filtered_points
