...
class Explorer_node(Node):
    def __init__(self):
        ...
        self.start_accepted_exploration_ratio = np.min([1.0,0.9*((circular_area_covered_by_camera/approximate_circular_area_covering_entire_area)* ((self.camera_angle)/(2*np.pi)))])
    ...
    def explore_next(self):
        ...
        result, x, y, yaw = self.generate_next_point(area,unexplored_nr, tot_nr_cells)
        area.one_point = True
        if result == "Fail":
            area.explored_once = True
            self.current_exploring_area +=1
        ...
        return "Not done", x, y, yaw
    
    #Generates points until a sufficiently good one is found
    def generate_next_point(self,area,unexplored_nr, tot_nr_cells):
        accepted_point = False
        accepted_exploration_ratio = self.start_accepted_exploration_ratio*tot_nr_cells/unexplored_nr
        accepted_exploration_ratio = np.min([1.0,accepted_exploration_ratio])
        nrTry = 0
        nr_to_high_claim = 0
        while accepted_point == False:
            ...
            accepted_point, fail_reason, accepted_x, accepted_y, accepted_yaw = self.check_point_profit(x,y, accepted_exploration_ratio,area,unexplored_nr)
            if fail_reason == "to high claim":
                nr_to_high_claim += 1
                self.get_logger().info(f"nr of to high claim: {nr_to_high_claim}")
                accepted_exploration_ratio *= 0.9
                self.get_logger().info(f"accepted exploration ratio: {accepted_exploration_ratio}")
                if nr_to_high_claim > 15:
    ...
    def check_point_profit(self,x,y, accepted_exploration_ratio,area,unexplored_nr):
        ...
        for point_on_circle_index in range(len(points_on_circle)):
            yaw = np.arctan2(y-points_on_circle[point_on_circle_index][1],x-points_on_circle[point_on_circle_index][0])
            v1 = self.camera_angle/2 + yaw
            v2 = -self.camera_angle/2 + yaw
            detection_edge = self.circle_creator.circle_filler_angle_dependent(points_on_circle[point_on_circle_index][0],points_on_circle[point_on_circle_index][1],area,self.explorer_grid,self.max_detection_range, False, False, True,True,v1,v2)

            if len(detection_edge) >= unexplored_nr*accepted_exploration_ratio:

                return True, None, points_on_circle[point_on_circle_index][0], points_on_circle[point_on_circle_index][1], yaw
    ...
    def turn(self, yaw):
        ...
        detection_edge = self.circle_creator.circle_filler_angle_dependent(current_x,current_y,None,self.safe_space_grid,self.max_detection_range-self.padding, True, False, False,True, v1, v2)
    ...
    def move_to_new_explore_point(self, obs_grid, goal_x, goal_y, goal_yaw):
    ...
    def run_explorer(self):
        ...
        while self.status != "All explored":
            ...
            if self.status != "Area explored" and self.status != "All explored":
                self.status, new_x, new_y, new_yaw = self.explore_next()
                if self.status == "Not done":
                    x.append(new_x)
                    y.append(new_y)
                    #input("Push a key to generate path")
                    obs_grid = self.path_planner_grid.copy()
                    self.move_to_new_explore_point(obs_grid, new_x,new_y, new_yaw)
...
def main():
    ...
    node.safe_space_grid = node.path_planner_grid.copy()
    node.turn()
    ...
    detection_edge = node.circle_creator.circle_filler_angle_dependent(current_x,current_y,None,node.safe_space_grid,node.max_detection_range-node.padding, True, False, False,True,None,None)
