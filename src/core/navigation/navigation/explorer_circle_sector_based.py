...
class Explorer_node(Node):
    def __init__(self):
        ...
        self.look_at_points_nr_dist_rejected_threshold = 200 # itterations before increment accepted dist
        ...
        self.look_at_points_dist_threshold_new_area = self.max_detection_range_grid_coordinates + np.round((self.approximate_area_size/2)/self.resolution,0)
        ...
        self.increment_threshold_yaw_look_at_point = 0.001 #rad
        ...
        self.attempt_detect_from_current_position_threshold_start = 400
        self.attempt_detect_from_current_position_threshold_new_area = 5
        ...
        self.look_from_points_nr_dist_rejected_threshold = 20 # itterations before increment accepted dist
        self.tot_look_from_points_nr_dist_rejected = 0
        self.look_from_points_nr_dist_rejected = 0
        self.increment_threshold_dist_look_from_point = np.round((self.approximate_area_size/8)/self.resolution,0) # cells
        ...
        self.nr_to_high_claim_threshold = 10 # itterations before reduce accepted exploration ratio
        self.nr_to_high_claim_rejected = 0
        self.tot_nr_to_high_claim_rejected = 0
        self.reduction_accepted_exploration_ratio  = 0.9 # cells
    ...
    def generate_next_point(self,area,unexplored_nr):
        ...
        while accepted_point == False:
            ...
            accepted_point, fail_reason, accepted_x, accepted_y, accepted_yaw = self.check_point_profit(x,y,area,unexplored_nr, dist, yaw_change, current_x, current_y)
        ...
        time_diff = tot_time.nanoseconds - (self.time_to_high_claim+self.time_look_from_point_dist_rejected+self.time_look_at_point_rejected_other_reason+self.time_attempt_detect_from_current_position+(self.get_clock().now().nanoseconds- self.start_time_generate_point.nanoseconds))
        self.get_logger().info(f"tot time: {tot_time.nanoseconds /1e9}")
        self.get_logger().info(f"time diff: {time_diff/1e9}")
        self.get_logger().info(f"Time: time_to_high_claim = {self.time_to_high_claim/1e9}, time_look_from_point_dist_rejected = {self.time_look_from_point_dist_rejected/1e9}, time_look_at_point_yaw_rejected = {self.time_look_at_point_yaw_rejected/1e9}, time_look_at_point_rejected_total= {self.time_look_at_point_rejected_other_reason/1e9}, time_look_at_point_dist_rejected = {self.time_look_at_point_dist_rejected/1e9}, time_attempt_detect_from_current_position = {self.time_attempt_detect_from_current_position/1e9}")
        self.get_logger().info(f"nr of to high claim: {self.tot_nr_to_high_claim_rejected}")
        ...
        self.get_logger().info(f"rejected reasons: look-at point too far away {self.look_at_points_nr_dist_rejected} times, look-at point too large angular difference {self.look_at_points_nr_yaw_rejected} times, look-from point too far away {self.tot_look_from_points_nr_dist_rejected} times,")
    ...
    def check_point_profit(self,x,y,area,unexplored_nr,dist, yaw_change, current_x, current_y):
        ...
        for point_on_circle_index in range(len(points_test_explore_from)):
            ...
            if dist_robot_point_on_circle <= self.look_from_points_dist_threshold:
                ...
                detection_edge = self.circle_creator.circle_filler_angle_dependent(points_test_explore_from[point_on_circle_index][0],points_test_explore_from[point_on_circle_index][1],area,self.explorer_grid,self.max_detection_range, False, False, True,True,v1,v2)
    ...
    def check_point_profit_less_explored(self,x,y, accepted_exploration_ratio,area,unexplored_nr):
        ...
        if area.one_point == False and self.path_planner_grid[int(area.midpoint_y),int(area.midpoint_x)] != -1 :
            ...
            points_on_circle = self.circle_creator.circle_filler_angle_dependent(x,y,area,self.path_planner_grid.copy(),self.optimal_detection_distance, False, True, False,False,None,None)   
    ...
    def full_turn(self):
        ...
        current_x, current_y = self.convert_to_grid_coordinates(current_x, current_y)
        detection_edge = self.circle_creator.circle_filler_angle_dependent(current_x,current_y,None,self.explorer_grid,self.max_detection_range, True, False, False,True,None,None)
        detection_edge = self.circle_creator.circle_filler_angle_dependent(current_x,current_y,None,self.safe_space_grid,self.max_detection_range, True, False, False,True,None,None)
        self.publish_grid_map(self.explorer_grid,"exp")
    ...
    def part_turn(self, yaw):
        ...
        v2 = -self.camera_angle/2 + yaw
        current_x, current_y = self.convert_to_grid_coordinates(current_x, current_y)
        detection_edge = self.circle_creator.circle_filler_angle_dependent(current_x,current_y,None,self.explorer_grid,self.max_detection_range, True, False, False,True, v1, v2)
        detection_edge = self.circle_creator.circle_filler_angle_dependent(current_x,current_y,None,self.safe_space_grid,self.max_detection_range, True, False, False,True, v1, v2)
        self.publish_grid_map(self.explorer_grid,"exp")
    ...
    def move_to_new_explore_point(self, obs_grid, goal_x, goal_y, goal_yaw, full_turn):
        ...
        waypoints, made_it_all_the_way = self.planner.waypoint_creator(True, self.safe_space_grid)
        for point in waypoints:
            x,y = point
            grid1[y,x] = 90
