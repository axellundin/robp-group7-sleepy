...
#################################################################
#################################################################
#Change in setup to run this node insted of hampus_odometry_for_testing_path
#################################################################
#################################################################
#################################################################
def quaternion_from_yaw(x, y, yaw):
...
class Odometry(Node):
    ...
    def __init__(self):
        super().__init__('odometry')
        print("original_odometry Imu dynamic wighting")
    ...
    def encoder_callback(self, msg: Encoders):
        ...
        self._yaw = self.alpha * (self._yaw_encoder) + (1 - self.alpha) * self._yaw_imu
